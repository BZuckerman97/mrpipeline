...
# Fixtures and the hf() wrapper live in helper-allele-fixture.R. Under the
# bug, every non-palindromic outcome beta is sign-inverted and eaf.outcome
# becomes the complement of eaf.exposure, while palindromic SNPs look
# untouched -- see the "palindromic" test below.
...
test_that("allele check errors by default on a swapped-allele outcome", {
  ...
  # Message is actionable: pair and counts, an offender, and the fix
  msg <- conditionMessage(err)
  expect_match(msg, "between \"exp\" and \"out\": 10/10")
  expect_match(msg, "rs[0-9]+ \\([ACGT]\\): eaf.exposure = ")
  expect_match(msg, "col_map")
...
test_that("palindromic betas are unchanged by the bug, so the check must ignore them", {
  ...
  ok <- hf(f$exposure, f$ok, allele_check = "none")
  bug <- hf(f$exposure, f$bug, allele_check = "none")
...
test_that("allele check passes on genuine EAF scatter and records a pass", {
  ...
  expect_no_condition(res <- hf(f$exposure, f$ok))
...
test_that("allele_check = 'none' reproduces plain harmonisation exactly", {
  ...
  expect_no_condition(res <- hf(f$exposure, f$bug, allele_check = "none"))
...
test_that("allele_check = 'warn' warns with its class and still returns data", {
  ...
  expect_warning(
    res <- hf(f$exposure, f$bug, allele_check = "warn"),
...
test_that("check_allele_orientation_gwas checks instruments plus shared SNPs", {
  ...
  # Outcome without EAF, with nothing in common, or empty -> skipped
  out_no_eaf <- f$bug
  out_no_eaf$eaf <- NULL
  out_disjoint <- f$bug
...
test_that("the bundled LD panel covers exactly the bundled CD40 SNPs", {
  prefix <- sub(
    "\\.bed$",
    "",
  ...
  # 1000 Genomes phase 3 EUR, generated by data-raw/ld_ref.R
  fam <- utils::read.table(paste0(prefix, ".fam"))
  expect_equal(nrow(fam), 503)
  expect_equal(fam$V1, fam$V2)

  bim <- utils::read.table(paste0(prefix, ".bim"), colClasses = "character")
  exposure <- cd40_exposure[order(cd40_exposure$pos.exposure), ]
  expect_equal(bim$V2, exposure$SNP)
  expect_equal(as.numeric(bim$V4), exposure$pos.exposure)
  expect_true(all(bim$V1 == "20"))
  panel_alleles <- paste(pmin(bim$V5, bim$V6), pmax(bim$V5, bim$V6))
  exposure_alleles <- paste(
    pmin(exposure$effect_allele.exposure, exposure$other_allele.exposure),
    pmax(exposure$effect_allele.exposure, exposure$other_allele.exposure)
  )
  expect_equal(panel_alleles, exposure_alleles)
  expect_setequal(bim$V2, sjogren_outcome$rsids)

  # .bed: 3 magic bytes + ceiling(503 / 4) bytes per SNP
  expect_equal(
    file.size(paste0(prefix, ".bed")),
    3 + nrow(bim) * ceiling(nrow(fam) / 4)
  )
})
