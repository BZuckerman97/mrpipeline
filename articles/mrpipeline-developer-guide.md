...
Defined in `R/last_allele_check.R`. Returns the record stored by the
most recent
...
pattern), or `NULL` with a message if none has run. The record is a
named list – `status` (`"pass"`/`"fail"`/`"skipped"`), `n`,
`n_complementary`, `prop`, `threshold`, `min_n`, `allele_check`,
`exposure`, `outcome`, `id.exposure`, `id.outcome`, `n_sampled`, and
`variants`, a data frame with one row per SNP that had both frequencies
(palindromic and `remove` rows included but flagged, with a `score`
column to sort offenders). A list rather than a data frame with
attributes so the summary fields survive `dplyr` verbs and `[`, and no
S3 class or print method is needed.

The storage is a package-level environment,
`.mrpipeline_env <- new.env(parent = emptyenv())` in `R/zzz.R` (the same
idiom as codeminer’s `.codeminer_env`). The namespace is locked on load
but bindings inside that environment are not, so helpers can assign
`.mrpipeline_env$last_allele_check` at run time. It is the package’s
only mutable state.
...
Single choke point for harmonisation, used by both
[`run_mr()`](https://github.com/BZuckerman97/mrpipeline/reference/run_mr.md)
and
[`run_coloc()`](https://github.com/BZuckerman97/mrpipeline/reference/run_coloc.md).
Wraps
...
runs
[`check_allele_orientation()`](https://github.com/BZuckerman97/mrpipeline/reference/check_allele_orientation.md)
on the raw output, returns a 0-row frame if harmonisation produced
nothing usable (so callers hit their `nrow == 0` early-return instead of
erroring on a missing `mr_keep` column), then filters to
`mr_keep == TRUE` and drops duplicate SNPs. The allele check runs
*before* the `mr_keep` filter so SNPs later dropped for a missing
`beta`/`se` still contribute their frequencies, and on every call so the
stored record is never stale.
[`run_mr()`](https://github.com/BZuckerman97/mrpipeline/reference/run_mr.md)
passes `check = FALSE` because it has already checked a larger set via
...
use `A1`/`A2` to mean REF/ALT with `BETA`/`FRQ` per `A2`;
[`format_gwas()`](https://github.com/BZuckerman97/mrpipeline/reference/format_gwas.md)
reads `A1` as the effect allele, so every beta is silently inverted.
Harmonisation is per-variant and letter-based, so it faithfully applies
the swapped labels and the output is internally consistent – the only
trace is that `eaf.outcome` ends up describing the other allele,
i.e. `eaf.outcome ~ 1 - eaf.exposure` across the set. The helper counts,
among *informative* rows (both EAFs present, `palindromic == FALSE`,
`remove == FALSE`, one row per SNP), how many have `eaf.exposure` closer
to `1 - eaf.outcome` than to `eaf.outcome`; it fails when that
proportion exceeds 0.70 with at least 10 informative rows, and is
`"skipped"` below that. Ties (`eaf.outcome == 0.5`) count as not
complementary; near-0.5 frequencies can only dilute the proportion,
never trigger it.

Palindromic SNPs are excluded because TwoSampleMR resolves their strand
*from* the frequencies: when effect allele and frequency are both
mis-assigned the two flips cancel, `eaf.outcome` looks consistent, and
the harmonised beta can be identical to the correct value while every
non-palindromic beta in the same set is inverted (the tests prove this
on the fixture). Including them would only dilute the statistic – and
once the check fails, every palindromic strand call in that pair is
unreliable regardless of its beta.

The check is symmetric: it cannot say which dataset is wrong. The
condition message (classes `mrpipeline_allele_check_error` /
`mrpipeline_allele_check_warning`) lists the five worst offenders
inline, the tie-breakers (reference panel frequencies, a known-direction
...
instrument set is often below the 10-SNP minimum (a cis-MR may have
three instruments), but
[`run_mr()`](https://github.com/BZuckerman97/mrpipeline/reference/run_mr.md)
holds the full exposure and outcome before it narrows the outcome to
instrument rsIDs. This helper harmonises the instruments plus up to 1000
further shared SNPs and hands the result to
[`check_allele_orientation()`](https://github.com/BZuckerman97/mrpipeline/reference/check_allele_orientation.md).
...
`inst/extdata/ld_ref.{bed,bim,fam}` — PLINK binary fileset of **real
1000 Genomes phase 3 genotypes** (release 20130502, GRCh37) for the 503
EUR individuals at the 50 `cd40_exposure` SNPs
(chr20:44666595–44782428). Total size ~6 KB. It is generated by
`data-raw/ld_ref.R`, which fetches only the CD40 slice of the public
chr20 VCF by remote tabix, checks every SNP’s position and alleles
against `cd40_exposure`, and writes the fileset with PLINK. Its
Bioconductor dependencies are not in `DESCRIPTION` (`data-raw/` is
build-ignored), and the fetched genotypes are cached under
`tools::R_user_dir("mrpipeline", "cache")`, so re-running needs no
network.
...
- **Clump leniently for cis-region LD tests.** At the default
  `rsq_thresh = 0.001` the CD40 region clumps to a single instrument;
  the
  [`run_mr()`](https://github.com/BZuckerman97/mrpipeline/reference/run_mr.md)
  tests use `rsq_thresh = 0.3` (seven instruments, \|r\| up to 0.53).
- **rs13045469 is tri-allelic in 1000 Genomes** (G/A/C; the exposure has
  C/G). The panel codes it as C vs not-C (A written as G), which is the
  exact LD of the C-allele dosage.
- **A1 is PLINK’s default (the minor allele).** The single-SNP test
  relies on rs1883832’s A1 being `"T"`; the generator asserts it.

For edge cases a real panel cannot pin down – a chosen r between a
chosen pair, or an exactly singular pair – use `make_ld_panel()` in
`tests/testthat/helper-ld-panel.R`, which simulates a small panel on the
fly.
