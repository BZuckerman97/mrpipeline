...
#' )
#' }
format_single_cell_onek1k <- function(onek1k_mapping, onek1k_cell_type) {
  # Input validation
  ...
  if (nrow(cell_type_info) == 0) {
    stop(
      "The specified `onek1k_cell_type` was not found in the mapping data frame."
    )
  }
  if (nrow(cell_type_info) > 1) {
    warning(
      "Multiple entries found for the specified `onek1k_cell_type`. Using the first one."
    )
